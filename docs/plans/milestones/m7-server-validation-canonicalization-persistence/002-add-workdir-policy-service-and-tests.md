...
## Deliverables
- [x] `apps/server/src/services/workdirPolicyService.ts` added with canonicalization and allowlist checks
- [x] `apps/server/src/services/sessionService.ts` calls policy service and persists normalized policy metadata
- [x] `apps/server/src/__tests__/workdirPolicyService.test.ts` and `apps/server/src/__tests__/sessionService.test.ts` cover accept/reject paths
