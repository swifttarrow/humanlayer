...
## Deliverables
- [x] `apps/agent/src/tools/workspacePolicy.ts` added with canonical path guards
- [x] `apps/agent/src/tools/fileTools.ts`, `patchTool.ts`, and `shellTool.ts` enforce readable/writable boundaries
- [x] `apps/agent/src/__tests__/stepLoop.test.ts` includes denied-access test coverage
