...
## Deliverables
- [ ] `apps/agent/src/main.ts` reads workdir policy from claimed session metadata
- [ ] `apps/agent/src/runner/stepLoop.ts` accepts policy context and passes it to tool executors
- [ ] Shell tool default `cwd` is derived from resolved working directory
