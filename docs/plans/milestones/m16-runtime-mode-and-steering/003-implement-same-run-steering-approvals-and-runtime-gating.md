...
## Deliverables
- [ ] `apps/server/src/routes/sessions.ts` exposes pause/resume/approve/reject/clarification endpoints on existing session attempts.
- [ ] `apps/agent/src/runner/stepLoop.ts` gates scheduling by run-control state and yields for clarification/approval when required.
- [ ] `apps/server/src/services/eventIngestService.ts` and `apps/ui/src/pages/SessionDetailPage.tsx` project and render steering/approval timeline events.
