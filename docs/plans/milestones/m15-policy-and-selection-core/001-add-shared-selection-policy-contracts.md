...
## Deliverables
- [ ] `packages/shared/src/contracts.ts` includes runtime mode policy (`local_only`, `docker_only`, `dual_mode`) and typed selection failure envelopes.
- [ ] Approval, pause/resume, provider/model selection, and tool/provider metadata contracts are added for downstream consumers.
- [ ] Contract changes are covered by shared package typecheck and any required contract tests.
