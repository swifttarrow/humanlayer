...
## Deliverables
- [x] Configurable limits for exploration reads/searches/lines and exploration-only steps
- [x] Budget accounting integrated into `apps/agent/src/runner/stepLoop.ts`
- [x] Exhaustion path emits terminal semantics that are not `session.completed` without a write attempt
